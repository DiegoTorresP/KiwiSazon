...
validador.agregarValidacion((req, res, next) => {
    body('recetaId').notEmpty().escape().trim().custom(async (value) => {
          // Realiza la verificación de duplicados
          const Receta = new RecetaDao();
          const receta = await Receta.getRecetaByID(value);
          if(receta){
            nombre = receta.platilloNombre;             
          }else{
            throw new Error('No se localizo la receta.') 
          }
        })(req, res, next);
  });
...
validador.agregarValidacion((req, res, next) => {
  body('platilloNombre').notEmpty().escape().trim().isLength({ min: 5 ,max:80}).withMessage('Ingrese un nombre de platillo correcto').custom(async (value) => {
        // Realiza la verificación de duplicados
        if(nombre!=''){
            if(nombre.toUpperCase() != value.toUpperCase()){
                const Receta = new RecetaDao();
